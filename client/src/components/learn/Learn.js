...
const Learn = () => {
  const [learnBlogs, setLearnBlogs] = useState([]);
  ...
  useEffect(() => {
    axios
      ...
      .then((response) => {
        ...
        const learnBlogs = blogs.filter((blog) =>
          blog.categories.some((category) => category.trim().toLowerCase() === 'learn')
        );
        setLearnBlogs(learnBlogs);
      })
      .catch((error) => {
        console.error('Error fetching learn blogs:', error);
      });
  }, []);
